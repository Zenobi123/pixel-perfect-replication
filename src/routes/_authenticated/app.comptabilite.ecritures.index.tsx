...
function EcrituresPage() {
  ...
  const { data: ecritures, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Ecriture[]> => {
      let q = supabase
        .from("ecritures")
        .select("id, numero, date_piece, reference, libelle, statut, journal_id, journaux(code, libelle), lignes_ecriture(debit, credit)")
        .eq("entreprise_id", current!.id)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{ecritures?.length ?? 0} dernières écritures</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Aucune écriture. Créez la première !</td></tr>
              ) : ecritures?.map((e) => {
                ...
                return (
                  <tr key={e.id} className="border-t hover:bg-muted/30">
                    ...
                    <td className="px-4 py-2">
                      <Link to={"/app/comptabilite/ecritures/$id" as never} params={{ id: e.id } as never} className="hover:underline">
                        {e.libelle}
                      </Link>
                      {e.reference && <span className="ml-2 text-xs text-muted-foreground">({e.reference})</span>}
                    </td>
